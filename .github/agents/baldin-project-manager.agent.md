...
tools: [vscode/getProjectSetupInfo, vscode/resolveMemoryFileUri, vscode/switchAgent, vscode/askQuestions, execute/testFailure, execute/getTerminalOutput, read/getNotebookSummary, read/problems, read/readFile, read/viewImage, read/readNotebookCellOutput, read/terminalSelection, read/terminalLastCommand, agent, agent/runSubagent, search/changes, search/codebase, search/fileSearch, search/listDirectory, search/textSearch, search/usages, web/fetch, browser/openBrowserPage, browser/readPage, browser/screenshotPage, browser/navigatePage, vscode.mermaid-chat-features/renderMermaidDiagram, todo]
agents:
...
- Baldin Design Lead Agent: archived-design reference owner for `operator-design`, browser-harness capture, repo-backed `.figma.ts` mapping, and historical design-to-code comparison.
...
- Archived-design reference work, `operator-design` maintenance, browser-harness capture, design-system promotion review, or repo-backed `.figma.ts` metadata alignment: Baldin Design Lead Agent.
