...
def test_create_lead(test_app_with_db):
    response = test_app_with_db.post("/leads/", data=json.dumps({"url": "https://foo.bar", "search_id": 1}))
    assert response.status_code == 201

    assert response.json()["url"] == "https://foo.bar"
    assert response.json()["search_id"] == "1"
