...
- Default to the local `docker-compose.yml` stack with `docker-compose up --build --watch` for routine development. Prefer one Compose Watch boot plus targeted smoke checks over repeated manual service startup.
...
- Backend changes should preserve FastAPI and OpenAPI correctness and use the narrowest useful tests or lint checks first. Prefer `./scripts/run_backend_pytest.sh` for DB-backed pytest and `pipenv run pytest` only for intentional host-side loops.
