...
const DirectoryPage: React.FC = () => {
  ...
  const setSuperuserView = useCallback(
    (nextValue: boolean) => {
  ...
  const refresh = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getAllDirectoryUsers(token, {
        superusers_only: superusersOnly,
      });
      setUsers(res ?? []);
    } catch (e: unknown) {
      notify(
        e instanceof Error ? e.message : 'Failed to load Discover',
        'error',
      );
    }
    setLoading(false);
  ...
  const filtered = useMemo(() => {
    return users.filter((u) => {
      ...
      const matchesPlacement =
        placementFilter === 'all' || u.placement_status === placementFilter;
  ...
  useEffect(() => {
    setPage(1);
  }, [deferredSearch, placementFilter]);
  ...
  const canRequestConnection = useCallback(
    (candidate: UserDirectoryRead) =>
          ...
          >
            <Box sx={{ flex: 1 }}>
              ...
              <Typography
                ...
                sx={{ mt: 0.75, maxWidth: 560 }}
              >
                Discover superusers who stay visible by default and accept
                one-to-one connection requests from any authenticated account.
              </Typography>
            </Box>
      ...
      {loading ? (
        ...
        users.length === 0 ? (
          <EmptyState
            ...
            title={
              superusersOnly
                ? 'No discoverable superusers yet'
                : 'No people in Discover yet'
            }
            description={
              superusersOnly
        ...
        <Grid container spacing={2}>
          {paged.map((u) => {
            const location = [u.city, u.state, u.country]
              .filter(Boolean)
              .join(', ');
            ...
            return (
              <Grid key={u.user_id} size={{ xs: 12, sm: 6, md: 4 }}>
                <Card sx={{ height: '100%' }}>
                  <CardActionArea
                    onClick={() => navigate(`/network/discover/${u.user_id}`)}
                  >
                      ...
                      <Stack spacing={1.5}>
                        ...
                        >
                          ...
                          {(u.skills_summary ?? []).length > 3 && (
                            <Chip
                              label={`+${(u.skills_summary ?? []).length - 3}`}
                              size="small"
                              variant="outlined"
                            />
                          )}
