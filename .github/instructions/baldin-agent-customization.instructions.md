...
applyTo: AGENTS.md, **/AGENTS.md, .codex/config.toml, .codex/agents/*.toml, .github/copilot-instructions.md, .github/AGENTIC_SURFACE.md, .github/agents/*.agent.md, .github/prompts/*.prompt.md, .github/instructions/*.instructions.md, .github/skills/*/SKILL.md, docs/docs/engineering/copilot-*.md
---
...
- Keep layers explicit:
  - root/scoped `AGENTS.md` = canonical provider-neutral repo baseline and local-delta instruction tree
  - `.github/copilot-instructions.md` = Copilot compatibility baseline that applies the relevant root/scoped `AGENTS.md` chain plus `.github/instructions/**`
  - `.github/instructions/*.instructions.md` = Copilot-scoped rules and alignment layer for the shared baseline
...
- Do not tell agents to load every scoped `AGENTS.md` file globally. Apply root `AGENTS.md` plus the relevant scoped chain for the current working path or target files.
- Do not add tracked `AGENTS.override.md` files unless a deliberate override policy, allowlist, and validator coverage are added first.
...
- Keep Baldin's Figma guidance aligned across root/scoped `AGENTS.md`, `.github/**`, and local-development docs: assume a Professional-plan workflow without a Dev-seat dependency, prefer the browser harness plus Figma MCP or basic inspection, and treat Code Connect publish as optional.
