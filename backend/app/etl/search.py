...
class LinkedIn(JobSearch):
    ...
    def redirect_job_search(cls) -> str:
        if not linkedin.search_endpoint.startswith(
...
class Glassdoor(JobSearch):
    ...
    async def bypass_signin(self) -> None:
        ...
        try:
            element = await self.chrome.element(
                '//*[@id="JAModal"]/div/div[2]/span', "xpath"
            )
            element.click()  # type: ignore
    ...
    async def expand_job_description(self) -> None:
        try:
            element = await self.chrome.element(
                '//*[@id="JobDescriptionContainer"]/div[2]', "xpath"
            )
            element.click()  # type: ignore
        except (ElementClickInterceptedException, NoSuchElementException):
            pass
    ...
    async def scrape_job_post(self, job_button: WebElement) -> dict[str, str]:
        try:
            ...
            job_desc = await self.chrome.element(
                '//*[@id="JobDescriptionContainer"]', "xpath"
            )
    ...
    async def paginate_search(self) -> bool:
        try:
            element = await self.chrome.element(
                './/div[@class="tbl fill padHorz margVert"]'
            )
            ...
            print(
                "Unable to paginate search results, no more pages found. Terminating search..."
            )
        return False

    async def scrape_search(self, keyword: str, pages: int = 1):
        await self.chrome.get(
            f"https://www.glassdoor.com/Job/jobs.htm?suggestCount=0&suggestChosen=false&clickSource=searchBtn&typedKeyword={keyword}&sc.keyword={keyword}&locT=&locId=&jobType=",
            wait=3,
        )  # noqa
        await self.bypass_signin()
